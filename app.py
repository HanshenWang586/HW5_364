...
app.config["SQLALCHEMY_DATABASE_URI"] = "postgresql://localhost/hanshenwHW5"
## Provided:
...
class UpdateButtonForm(FlaskForm):
	newitem = TextAreaField("Enter your new TODO list item in the following format: Description, Priority -- separated by newlines")
...
def all_lists():
    ...
    for l in lsts:
    	lst = TodoList.query.filter_by(id=l.id).first()
    	all_lists.append((l.id,l.title))
    return render_template('all_lists.html',todo_lists=all_lists, form=form)
...
def update(item):
    ...
    if form.validate_on_submit():
        newPriority = form.newPriority.data
